...
export const registerUser = asyncHandler(async (req, res) => {
  ...
  const effectiveRole =
    normalizedRole === "admin" ? "user" : normalizedRole || "user";
...
export const verifyEmail = asyncHandler(async (req, res) => {
  ...
  if (!token) {
    res.status(400);
    throw new Error(
      "Verification token is required. Use the full link from the email.",
    );
  }
  ...
  if (!user) {
    if (process.env.NODE_ENV !== "production") {
      console.warn(
        "Verify email: no user found for token length",
        token.length,
        "(expected 64). Link may have been used already or from a different environment.",
      );
    }
    res.status(200).json({
      success: true,
      message:
        "This link was already used or has expired. If you can log in, your email is already verified.",
    });
    return;
  }
  if (
    ...
    res.status(400);
    throw new Error(
      "Verification link has expired. You can still log in; consider signing up again if you need a new link.",
    );
  }
...
export const forgotPassword = asyncHandler(async (req, res) => {
  ...
  if (user) {
    ...
    const { text, html, subject } = getPasswordResetEmail({
      name: user.name,
      resetUrl,
    });
    const emailResult = await sendMail({
...
export const resetPassword = asyncHandler(async (req, res) => {
  ...
  if (user.resetPasswordExpires && new Date() > user.resetPasswordExpires) {
    res.status(400);
    throw new Error(
      "Reset link has expired. Request a new one from the forgot password page.",
    );
  }
  ...
  if (req.body.avatar !== undefined && req.body.avatar) {
    try {
      if (req.body.avatar.startsWith("data:image/")) {
        const gcsUrl = await uploadAvatarToGCS(
          req.body.avatar,
          user._id.toString(),
        );
        if (gcsUrl) {
...
export const changePassword = asyncHandler(async (req, res) => {
  ...
  const user = await User.findById(req.user._id).select(
    "+password +passwordChangeToken +passwordChangeExpires",
  );
  ...
  const passwordChangeExpires = new Date(
    Date.now() + PASSWORD_CHANGE_EXPIRY_MS,
  );
  ...
  res.json({
    message:
      "Please check your email to confirm the password change. The confirmation link expires in 1 hour.",
    email: user.email,
  });
});
...
export const confirmPasswordChange = asyncHandler(async (req, res) => {
  ...
  if (!user) {
    res.status(400);
    throw new Error(
      "Invalid or expired confirmation link. Request a new password change from your profile.",
    );
  }
  ...
  if (user.passwordChangeExpires && new Date() > user.passwordChangeExpires) {
    res.status(400);
    throw new Error(
      "Confirmation link has expired. Request a new password change from your profile.",
    );
  }
