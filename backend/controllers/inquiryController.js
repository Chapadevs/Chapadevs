...
import {
  ...
  getInquiryUserConfirmationEmail,
...
export const submitInquiry = asyncHandler(async (req, res) => {
  ...
  if (!adminResult.success) {
    ...
    res.status(503);
    throw new Error(
      "Unable to send inquiry. Please try again later or contact us directly.",
    );
  }
  ...
  const { text: userText, html: userHtml } =
    getInquiryUserConfirmationEmail(userData);
  ...
  res.status(201).json({
    success: true,
    message:
      "Thank you! Your inquiry has been submitted. We've also sent you a confirmation email. We'll get back to you within 24 hours.",
  });
});
