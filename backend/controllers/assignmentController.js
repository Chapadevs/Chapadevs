...
export const getProjectDescriptionPublic = asyncHandler(async (req, res) => {
  ...
  if (!isAvailable) {
    return res
      .status(403)
      .json({ message: "Project description is not available" });
  }
...
export const acceptProject = asyncHandler(async (req, res) => {
  ...
  if (
    !project.assignedProgrammerIds.some((id) => id.toString() === userIdStr)
  ) {
...
export const leaveProject = asyncHandler(async (req, res) => {
  ...
  if (isPrimaryAssigned) {
    ...
    if (
      project.assignedProgrammerIds &&
      project.assignedProgrammerIds.length > 0
    ) {
...
export const removeProgrammerFromProject = asyncHandler(async (req, res) => {
  ...
  if (!isClientOwner && !isAdmin) {
    res.status(403);
    throw new Error(
      "Only the project client or an admin can remove a programmer from the project",
    );
  }
  ...
  if (isPrimaryAssigned) {
    ...
    if (
      ...
      const remaining = project.assignedProgrammerIds.filter(
        (id) => id.toString() !== userId,
      );
  ...
  await logProjectActivity(
    ...
    "project",
    project._id,
    { removedProgrammerId: programmerId },
  );
  await createNotification(
    programmerId,
