...
app.get("/api/vertex-ai/status", async (req, res) => {
  try {
    const vertexAIService = (await import("./services/vertexAI/index.js"))
      .default;
    const status = vertexAIService.checkVertexAIStatus();
    res.status(200).json({
      ...status,
      warning: !status.initialized
        ? "Vertex AI is NOT working - using mock data"
        : null,
...
async function start() {
  ...
  if (!connected) {
    console.warn(
      "⚠️ Server starting without database connection. Auth/protected routes will return 503.",
    );
  }
