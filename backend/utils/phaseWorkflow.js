...
export async function canProceedToNextPhase(projectId, currentPhaseId) {
  ...
  const incompleteSubSteps =
    currentPhase.subSteps?.filter((s) => !s.completed) || [];
...
export function getPhaseRequirements(phase) {
  const requiredQuestions =
    phase.clientQuestions?.filter((q) => q.required) || [];
...
export function validatePhaseCompletion(phase) {
  ...
  if (phase.requiresClientApproval && !phase.clientApproved) {
    return {
      valid: false,
      reason: `Phase "${phaseTitle}" requires client approval before completion`,
    };
  }
  ...
  for (const subStep of subSteps) {
    const isCompleted =
      subStep.status === "completed" || subStep.completed === true;
    if (!isCompleted) {
      return {
        valid: false,
        reason: `Complete all sub-steps in phase "${phaseTitle}" before marking complete`,
      };
    }
  }
  ...
  for (const subStep of subSteps) {
    ...
    if (missingSubAttachments.length > 0) {
      return {
        valid: false,
        reason: `Provide all required attachments for sub-steps in phase "${phaseTitle}" before marking complete`,
      };
    }
  }
...
export function getDefaultQuestionsForPhase(phaseTitle) {
  ...
  if (title.includes("design")) {
    ...
    questions.push(
      ...
      {
        question:
          "Do you have all the necessary credentials and documentation?",
        required: false,
        order: 3,
      },
    );
