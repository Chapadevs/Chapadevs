...
export function derivePhaseDatesFromSubSteps(subSteps) {
  ...
  for (const s of sorted) {
    ...
    if (startDate && dueDate) {
      ...
      startDates.push(startDate);
      const derivedDue = new Date(
        startDate.getTime() + durationDays * MS_PER_DAY,
      );
...
export function shiftSubStepsToStartAt(subSteps, newStartDate) {
  if (!Array.isArray(subSteps) || subSteps.length === 0 || !newStartDate)
    return subSteps;
  const toDate = toDateUtc;
  const toIso = (d) =>
  ...
  return sorted.map((s) => {
    ...
    return {
      ...s,
      startDate: start
        ? toIso(new Date(start.getTime() + deltaMs))
        : s.startDate,
