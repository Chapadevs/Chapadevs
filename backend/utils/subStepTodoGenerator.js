 ...
 */
export function generateSubStepTodos({ title = '', estimatedDurationDays = 1, deliverables = [] }) {
  const lower = (title || '').toLowerCase()
  const days = Math.max(1, Math.min(7, Math.round(Number(estimatedDurationDays) || 1)))
  ...
  if (lower.includes('wireframe') || lower.includes('design') || lower.includes('mockup')) {
    const tasks = [
  ...
  if (lower.includes('deploy') || lower.includes('launch') || lower.includes('handoff')) {
    const tasks = [
  ...
  if (lower.includes('integrat') || lower.includes('api') || lower.includes('wiring')) {
    const tasks = [
      'Configure endpoints',
      'Implement data flow',
  ...
  return defaultTasks.slice(0, count).map((text, i) => ({ text, order: i + 1 }))
}
