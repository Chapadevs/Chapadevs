...
export function validateAttachmentForRequired(buffer, mimetype, constraints) {
  ...
  const { minWidth, maxWidth, minHeight, maxHeight, allowedTypes } =
    constraints;
  ...
  if (
    ...
    if (minWidth != null && width < minWidth) {
      return {
        valid: false,
        error: `Width must be at least ${minWidth}px. Got ${width}px.`,
      };
    }
    if (maxWidth != null && width > maxWidth) {
      return {
        valid: false,
        error: `Width must be at most ${maxWidth}px. Got ${width}px.`,
      };
    }
    if (minHeight != null && height < minHeight) {
      return {
        valid: false,
        error: `Height must be at least ${minHeight}px. Got ${height}px.`,
      };
    }
    if (maxHeight != null && height > maxHeight) {
      return {
        valid: false,
        error: `Height must be at most ${maxHeight}px. Got ${height}px.`,
      };
    }
  }
