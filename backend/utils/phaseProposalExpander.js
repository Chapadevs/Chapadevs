...
const PHASE_TO_SUBSTEPS = {
  ...
  requirements: ["Document scope", "Define milestones", "Review and approve"],
  planning: [
    "Discovery session",
  ...
  qa: ["Run test suite", "Document results", "Regression testing"],
  launch: ["Production deployment", "Client handoff", "Post-launch checklist"],
  handoff: ["Deploy and verify", "Documentation", "Training session"],
};
...
export function expandPhasesWithSubSteps(phases, project) {
  ...
  const totalDays =
    dateDuration?.totalDays ?? (parseInt(project?.timeline, 10) || 8) * 7;
  const phaseCount = Math.max(1, phases?.length ?? 1);
  const daysPerPhase = Math.max(1, Math.floor(totalDays / phaseCount));

  return (phases || []).map((p, i) => {
    const existingSubSteps = Array.isArray(p.subSteps)
      ? p.subSteps.filter((s) => (s?.title || "").trim())
      : [];
        ...
        subSteps: existingSubSteps.map((s, j) => ({
          ...
          todos:
            Array.isArray(s.todos) && s.todos.length > 0
              ? s.todos
                  .map((t, k) => ({ text: t.text ?? "", order: k + 1 }))
                  .filter((t) => t.text)
              : generateSubStepTodos({
                  title: s.title,
                  estimatedDurationDays: Math.max(
                    1,
                    daysPerPhase / Math.max(1, existingSubSteps.length),
                  ),
    ...
    const subSteps = subStepTitles.map((title, j) => {
      const estimatedDays = Math.max(
        1,
        Math.floor(daysPerPhase / subStepTitles.length),
      );
